...
const Rows = ({files}) => {
  ...
  const html = [ onecard, onecard, onecard ].map((card, id) => (
      <Row key={id}>
        <td>{card.title}</td>
        <td>{card.dateOfChange.getDate()} {month[card.dateOfChange.getMonth()]} {card.dateOfChange.getFullYear()}</td>
      </Row>
  ))
  return html
}   
  ...
  return (
    <Body>
      <Info>
        ...
        <HideButton folder active> <Folder style={{ fontSize: "26px" }}/> Папки</HideButton>
        <HideButton folder> <Clock style={{ fontSize: "26px" }}/>  Недавнее</HideButton>
        <HideButton> <Person style={{ fontSize: "26px" }}/> Доступные мне</HideButton>
      </Info>
      <MTable striped>
        <thead>
          <tr>
