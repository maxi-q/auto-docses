...
export const LoadBox = (props) => {
    ...
    return<>
        <LoadDivStyled>
            ...
            <LoadBoxStyled>
                <NoneInput ref={input => {props.setInputElement(input)}} multiple="multiple" onChange={props.changeHundler} type="file" id="file" placeholder="Search"/>
                <div>
                    <OpenButton onClick={props.openClick}>Открыть</OpenButton>
                    {props.upload ? (<UploadButton onClick={() => props.sumbitFiles(props.files)} >Загрузить</UploadButton>) : nope()}
                </div>
                {props.upload ? (
                    <Preview> 
                        {props.url.map((urla, i) => {
                            return(
                                <Webviewer key={i} ref={props.viewer}></Webviewer>
                            )
                        })}
                    </Preview>
